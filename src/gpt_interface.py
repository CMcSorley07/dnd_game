...
class AIInterface:
    def __init__(self, model_name="gpt-3.5-turbo", base_url="https://api.openai.com/v1"):
        ...
        if not self.api_key:
            raise RuntimeError("Missing OPENAI_API_KEY environment")
    ...
    def send_message(self, message, system=None, example_type=None, debug=False):
        if system is None:
            ...
            if example_type and example_type in self.example_responses:
                system += f"\n\nExample - {example_type.capitalize()} Description: \n{self.example_responses[example_type]}"
        ...
        for attempt in range(3): 
            try:
                if debug:
                    print("Payload:", json.dumps(payload, indent=2)) # Only prints if debug=True
